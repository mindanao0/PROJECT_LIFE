...
def sha256_hex(value: object) -> str:
    digest: str = hashlib.sha256(canonical_bytes(value)).hexdigest()
    return digest
...
def audit_event_payload(run_id: str | None, sequence_no: int, actor: str, event_type: str,
...
def verify_audit_chain(events: list[dict[str, object]]) -> bool:
    ...
    for index, event in enumerate(events):
        ...
        payload = audit_event_payload(
            cast("str | None", event["run_id"]),
            index,
            cast(str, event["actor"]),
            cast(str, event["event_type"]),
            cast(str, event["payload_artifact_id"]),
            cast(str, event["created_at_utc"]))
